...
export type PanelView = {
   ...
   *  answer, and it is what `core:sample-measures` selects a panel on. Never blur them: they are two
   *  features wearing one mark. */
  trend?: 'history' | 'activity'
  /** The lookback for the delta beside the number. A POINT looked up, never a window aggregated. */
  compare?: 'day' | 'week'
  /** Which direction is good — NOT guessable, because open PRs going up is bad for one person's
   *  board and good for another's. Absent renders the delta in neutral ink rather than a guessed
   *  green. Display config on the PANEL (the user's judgement), deliberately unlike units and tones,
   *  which are the plugin's facts and hang off the field. */
  good?: 'up' | 'down'
}
