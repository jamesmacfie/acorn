...
// Pure derivations behind the add/remove/reorder picker. See docs/dashboards.md § The generated
// editor for why this logic lives outside the components.
 ...
 *
 *  Spelled structurally instead of as a `Pick` of the client registry's contribution type, because
 *  this package sits below both runtimes and can't name a client registry type. Every call site still
 *  passes the real contribution: the three fields are the same by construction. */
 ...
 *  nothing are offered, exactly the promise the editor makes. */
 ...
 *  swap. Never returns a kind the schema can't support: the editor offers no invalid choice, so
