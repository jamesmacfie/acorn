...
import { ensureBoundIdentity } from './core/identity'
...
const migrationsFolder = (() => {
  const packaged = resourcesPath ? join(resourcesPath, 'migrations') : null
...
export function openDb(dbPath: string): AppDatabase {
  ...
  // enabling enforcement would be a misleading no-op. server/storage/sqlite.ts holds it off explicitly,
...
export function makeBindings({ dbPath, blobsDir, nodeId, appVersion, capabilities }: BindingsOptions): RuntimeBindings {
  ...
  // its own into the data root instead of refusing to start (server/sessionKey.ts).
