...
// Settings → Schedules (docs/schedules.md). The whole node-to-client contract for periodic work: one
// merged list over the three declarers, plus pause, retune, run now and delete.
//
...
// not reach it.
//
// The scheduler throws BridgeError for every owner-fixable refusal, so viaBridge turns each into its own
// 4xx and nothing here maintains a status table.
...
export const schedules = new Hono<AppEnv>()
  ...
  // What the creation flow may offer, and nothing else. Read straight off the node-side registry rather
  // than through the scheduler bridge, because this is a question about what can be scheduled.
  //
