...
// The request-side half of the audit trail. Kept apart from server/audit.ts so THAT module stays free of
// Hono: it is also called from auth/deviceTokens.ts, which has a database and no request.
...
// answerable from the trail. An internal principal deliberately records its SCOPE rather than its task or
...
// promise recordAudit's fire-and-forget shape exists to make, undone one layer up — and it showed up
