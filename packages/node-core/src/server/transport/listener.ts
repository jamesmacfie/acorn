...
import { configuredPort, devDataDir } from './listenerConfig'
...
export function startListener(
  ...
  const upgradeDeps: TunnelDeps = {
    ...
    // configuration surface (server/transport/tunnelPorts.ts).
  ...
  // upgrade neither of them owns is destroyed here rather than left open forever. See server/transport/upgradeClaim.ts
...
// One definition of the on-disk app-data layout under a data root (DB, blobs). Both composition roots
// build their runtime through it: the supervised service peer and the plain-Node `dev:node` entry.
//
