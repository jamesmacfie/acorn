...
import { setRunBridge, type RunBridge } from '../routes/plugins/harness'
import { declaredTunnelPorts, loopbackPortOf } from './tunnelPorts'

// The allowlist is derived from what the node already resolves for a task, never configured
// (server/transport/tunnelPorts.ts). This is the one piece of it with edges worth pinning: which URLs
// imply a tunnellable port.
