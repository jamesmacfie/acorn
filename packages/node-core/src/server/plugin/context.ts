...
//
// It used to be a literal inside the loop in host.ts, which is the only place that needed it at boot, but
// a plugin's test had no way to get one, so thirteen plugins hand-forged
// `{ ... } as unknown as NodePluginContext` and the forgeries drifted from this shape in silence.
// testkit/pluginContext.ts calls this function, so a test context cannot drift from the boot context: it
// is the same object, over a temp data root.
//
...
export type LoadedPluginBinding = {
  ...
  // And what it declared as collections, for the same reason and by the same route: the node-side read
  // registry is synthesised from these `items` paths (../collections/registry.ts), so a loaded plugin's
  // panels can be sampled without a client and without the plugin shipping node code.
  collections?: readonly PluginCollectionDescriptor[]
  // And its commands, of which the host reads exactly one thing: which are `runNodeAction`, so a person
  // can put one on a schedule (../nodeActions/registry.ts). The rest of a command, its palette entry, its
  // keybinding, its category, is the client's business and never reaches here.
...
export type PluginContextOptions = {
  ...
  // does not need it, since the caller never reads `migrationsModule` for a loaded plugin (host.ts:112),
  // so the derivation is the caller's single responsibility and there is one source of truth here.
  storage?: PluginStorage
  ...
  // takes it back on re-init or on a contained failure. A caller that passes nothing (a test) simply keeps
  // no undo record.
  onUndo?: (undo: () => void) => void
  // The candidate registration set (server/plugin/host.ts § reload). When present, every registration this
  // context hands the plugin is buffered here as a thunk instead of reaching the module-singleton
  // registries, and the host replays it only once it has decided to commit.
  //
  // It exists because all six registries reject a duplicate: tool names, provider ids, capability ids, and
  // on a reload the previous instance's registrations are still in them. Running the candidate's init
  // straight at the live registries would throw on the plugin's own name, which would make "if init
  // throws, the previous registrations stay fully live" unreachable rather than merely untested.
  pending?: (() => void)[]
}
...
export function buildPluginContext(options: PluginContextOptions): NodePluginContext {
  ...
  const ctx: NodePluginContext = {
    name: plugin,
    routes: {
      // Absent for a loaded plugin: handing the host a live Hono instance from another realm is exactly
      // what cannot survive the process boundary rung 2 puts there (docs/security.md § Design rules).
      // `undefined as never` rather than a throwing stub, so the failure is the immediate "not a function"
    ...
    // composition root, which owns its start and stop, and a context built before it exists must still
    // work.
    //
    ...
    schedules: {
      register: (schedule) => {
        ...
        // Dispose removes the definition and keeps the state row, which is what makes a plugin's lifecycle
        // non-destructive: its pause and its history are waiting when it comes back.
        recordUndo(() => handle.dispose())
      },
    },
    // Owner-bound like routes and tools, and for the sharper version of the same reason: this pointer is
    // what the measure sampler dispatches through, so a collection filed under a stranger's name would
    // have the node reading one plugin's route under another's badge. The route itself is re-confined on
    // every call (../collections/registry.ts), which is the belt to this brace.
    collections: {
      register: (collection) => registerCollectionRead({ ...collection, pluginId: plugin }),
    },
    // Owner-bound for the sharpest version of the reason yet: this is the list a person picks a scheduled
    // action from, and the tier beside each entry is what the confirmation they accept is drawn from. A
    // plugin filing one under a stranger's name would be borrowing that plugin's reputation for its own
    // route.
    nodeActions: {
      register: (action) => registerNodeAction({ ...action, pluginId: plugin }),
    },
    // Owner-bound like the three above, and here it is also the provenance the dialog draws: every concern
    // this check answers with is rendered beside the plugin's name, so filing one under a stranger's name
    // would be putting words in that plugin's mouth in front of the owner.
    taskChecks: {
    ...
    // facets its manifest declared. main/pluginPermissions.ts explains what that does and does not buy. It
    // is least privilege for cooperative code, not a security boundary.
    ...
    events: {
      ...
      // Never present for a loaded plugin, whatever its manifest says. PTY stream ownership and WS channel
      // prefixes are infrastructure that exactly one plugin may own, and neither survives a message-passing
      // boundary (README § Two tiers).
      channel: permissions
        ? (undefined as never)
        : (prefix, handler) => {
  ...
  //
  // One post-pass rather than a wrapper at each of the ten registration sites, because every site above is
  // contextually typed by NodePluginContext and wrapping them inline would cost every parameter an
  ...
  // candidate set when there is one. `events` and `storage` are not registrations, since a broadcast has
  // to go out when it is sent and `storage.open()` has to return a handle, so those are only revoked. That
  // is exact rather than convenient for the tier reload touches: `events.channel` and `streams` are the
  // only registrations on `events` and both are absent for a loaded plugin.
  let revoked = false
  ...
  // and after it (dispose the real one). `get`, `require` and `ids` are reads and stay exactly as they
  // are.
  if (pending) {
