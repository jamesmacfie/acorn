...
export type PluginsBridge = {
  ...
  // Every package on disk right now, including the client-only ones the host never saw. Separate from
  // `roster()` because the roster describes what this process assembled, and a package with no node half
  // never enters the plugin host at all, but its bundle is exactly what phase 2 distributes, so it still
  // has to appear.
  //
  ...
  // (server/plugins/reload.ts). It throws for a name this node did not load from disk; a plugin whose new
...
// value that arrives through a variable, so a field added to `InstalledPluginInfo` for the node's own use
// would ship to every paired device, silently, forever.
//
// This is the check that makes it fail closed instead. Adding a field to either side is now a build error
// that says so, and the fix is a deliberate one line: name it in the `Omit` above to keep it on the node,
// or add it to `InstalledPluginRow` to publish it on purpose.
...
// The running plugin set, plus the pending one. They differ exactly when a toggle has been saved and the
// node has not restarted, which is the state the UI has to render.
export const pluginState = (bridge: PluginsBridge): { plugins: NodePluginRow[]; restartRequired: boolean; requests: PluginApprovalRequest[] } => {
  ...
  //     banner: its contributions are all client-side, and the client re-initialises its plugin host on a
  //     roster change (apps/desktop client/activate.ts disposes-then-registers).
  ...
  for (const failure of loadFailed.values()) {
    ...
    // and the compiled-in one steps aside, but only if the disk copy actually loads), or a second directory
    // claiming an id the first one won. Both used to end here silently, which is the worst of the three:
    // the row said 'active' and it was true, so nothing anywhere said that the copy the owner installed is
    // not the code they are running.
    //
