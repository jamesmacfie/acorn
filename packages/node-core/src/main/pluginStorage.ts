...
export function openPluginDb(dataDir: string, plugin: string, options: { migrationsFolder: string }): PluginDatabase {
  ...
  // keys, and main/sqlite.ts keeps enforcement off to match better-sqlite3, so no plugin gets
...
// (main/pluginLoader.ts). Only the chain differs: a built-in's ships with the app and resolves from
...
export function builtinPluginStorage(dataDir: string, plugin: string, moduleUrl: string): PluginStorage {
  ...
  // (main/pluginMigrations.ts). Checked here because fileURLToPath otherwise throws a bare "Invalid
