...
// SIGTERM lets a shell run its traps and a compose stack stop its containers; SIGKILL after this stops a
// process that ignores it from wedging the caller forever.
export const KILL_GRACE_MS = 2_000

export type ProcSpec = {
  file: string
  args?: readonly string[]
  // Required, and required to be absolute by the callers that have a worktree: an inherited cwd is how a
  // task-scoped command ends up running against the wrong checkout.
  cwd: string
  // Merged over the allowlisted base. Callers pass buildSessionEnv() output here; applying childEnv twice
  // is idempotent.
  env?: Record<string, string>
  // Exact names or `PREFIX_*` globs to carry over from the parent environment in addition to the base
  // allowlist. Use for tool configuration (DOCKER_HOST, GIT_*), never for credentials.
...
export type ProcResult = {
  ...
  timedOut: boolean
  aborted: boolean
  // At least one stream hit maxOutputBytes. The process was not killed for it; some callers want the side
  // effect to complete and only sample the output.
  truncated: boolean
  // Set when the process could not be started at all (ENOENT for a missing binary). Distinguishing this
  // from "ran and failed" is what lets a caller say "docker is not installed".
  spawnError: string | null
}

// Which parent variables a child may see. Everything not named here is absent, including every ACORN_*
// token the node injects into its own process and every secret binding.
export function brokerEnv(spec: Pick<ProcSpec, 'env' | 'passthrough'>, parent: NodeJS.ProcessEnv = process.env): Record<string, string> {
  ...
  for (const rule of spec.passthrough ?? []) {
    if (rule.endsWith('*')) {
      ...
      // A bare '*' would copy the whole parent environment, secrets included, which would quietly restore
      // the behaviour this allowlist exists to remove. Refuse it rather than trust that no caller writes
      // it.
...
export function runProcess(spec: ProcSpec): Promise<ProcResult> {
  ...
  return new Promise((resolve) => {
    ...
    const signalGroup = (signal: NodeJS.Signals): void => {
      try {
        // A negative pid is the whole group. Falls back to the direct child if the group is already gone
        // (ESRCH) or the platform refuses.
    ...
    //
    // Decoding each chunk with `chunk.toString()` corrupts any multi-byte character that straddles a pipe
    // chunk boundary: a 64 KiB read can split a 3-byte character in half and each half decodes to U+FFFD.
    // Preserve partial sequences across chunks with a StringDecoder before exposing text to callers.
    //
    // Accumulating Buffers also makes the cap exact. Slicing a decoded string to a byte length cuts
    // mid-character, so the result could exceed maxOutputBytes and end in U+FFFD; slicing the byte stream
    // cannot overshoot.
    ...
    const settle = (result: Omit<ProcResult, 'stdout' | 'stderr' | 'timedOut' | 'aborted' | 'truncated'>): void => {
      ...
      // fired, and clearing the timer at that moment left the survivor running forever. Confirmed with a
