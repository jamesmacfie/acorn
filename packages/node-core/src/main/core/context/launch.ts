// The launch-context seam (CoreServices.context). Two reads that a plugin cannot make for itself
// once it has no handle to core's database:
//
//   - the `prefs` row that turns startup context injection off, and
//   - `agentTools/contextSections.ts`'s assembler, which walks core's contribution registry and
//     reads the github/linear/rollbar mirrors on the way.
//
// One caller: plugins/memory's launchInjector, which pushes "PR + linked issues + notes + repo
// memory" into a fresh agent session as its first prompt (docs/notes-and-memory.md). Which
// sections, how they are formatted, and whether to send at all stays a policy decision in the
// plugin; only the two reads that need core's tables live here.
...
export type ContextService = {
  // The owner's `startup_context_injection` pref. Defaults to true: absence means the owner never
  // changed it.
