...
// Core owns the machine identity. It used to be bound by a feature plugin: plugins/github's device-flow
// route set it after connecting an account, which made core's answer to "who is the user" a side effect
// of connecting one provider, and left every internal caller dead until GitHub was connected once.
//
// The identity is minted at boot now: an opaque `owner-<uuid>` written the first time a node starts with
// nothing bound. Installs that bound a GitHub login under the old scheme keep it as their opaque owner
// id forever, so no data rewrite is needed. Providers never bind; a GitHub login is metadata on its
// integration row.
...
// principal used to fall back to '' before anything was bound, and those rows belong to the owner this
