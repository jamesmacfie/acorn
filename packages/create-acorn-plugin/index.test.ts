...
// @ts-expect-error — the scaffold is published standalone with zero dependencies, so it is plain
...
// The scaffold is a COPY of the authoring contract living outside the repository's reach: nothing a
// reader of packages/protocol would think to grep finds it, and a stranger's first plugin is what
// breaks when it drifts. These are the three ways it can drift silently.
...
it('emits a manifest the host parses, cross-field rules and all', () => {
  ...
  // an openPane naming a pane that is not declared, a route outside the plugin's namespace.
...
it('emits a node half that loads and satisfies the structural plugin check', async () => {
  ...
  try {
    ...
    // The client half cannot be imported here — it reaches for `document` at module scope, which is the
    // point of it. Parse it instead: a syntax error in the generated bridge is otherwise a blank
    // rectangle in someone else's app.
