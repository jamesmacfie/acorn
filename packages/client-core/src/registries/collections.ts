...
// manifest and the descriptor pass synthesises the same contribution over the host's reader
// (plugins/chrome/register.ts). Nothing downstream can tell which feeder a collection came from, which
// is the point — a third-party plugin's panels ship no client bundle and are pixel-identical to a
// first-party one's.
//
// Deliberately NOT merged with `nodeStats`, for the reason that registry gives against merging with
// `attention`: a stat is one integer with a label, a collection is a schema plus rows. Forcing them
// into one shape would mean every stat inventing an empty schema.
export type CollectionContribution = {
  // `<pluginId>:<collectionId>`, minted by `collectionKey` below and never spelled by hand. The
  // registry needs one string; the WORLD addresses a collection as `(pluginId, collectionId)` and by
  // nothing else, so the two halves stay separately readable on the contribution.
  id: string
  pluginId: string
  collectionId: string
  name: string
  ...
  // Seconds, the manifest's own bound. A hint for whatever places this collection; nothing here polls.
  refresh?: number
...
/** The answer for "nothing to show", and the answer for an unusable one. Deliberately the same value:
 *  a panel that renders an empty table has told the reader everything either case gives it, and the
 *  reason a fetch produced nothing is a console line for the plugin's author, not a second empty
 *  state for the reader to interpret. */
