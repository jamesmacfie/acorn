// Which model a connection starts on. Re-exported rather than defined here: the Database plugin's node
...
//
// A `.ts` of its own rather than a line in ./ModelConnectionPicker.tsx, so a plugin bundle that draws a
