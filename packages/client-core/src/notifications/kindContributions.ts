...
export const noticeKindContributions: NoticeKindContribution[] = [
  ...
  // Anything a sandboxed plugin frame raises through the bridge's `toast` verb (docs/plugins.md).
  // `toast: false` on purpose: an OS notification fires whether or not the window has focus, and
  // third-party code shouldn't be able to put text on the user's desktop.
