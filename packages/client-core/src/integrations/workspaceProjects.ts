...
//
// The host's own map (settings/ConnectionProjectMap.tsx) doesn't use these: it writes one connection's
// rows at a time through the connection-scoped route, which cannot reach a sibling's rows at all.
// These stay because a bulk provider-scoped replace over the workspace-side route is still the right
// shape for a plugin doing one, and they're the plugin-facing form exported through
