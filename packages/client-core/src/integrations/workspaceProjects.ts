...
//
// The host's own picker (settings/WorkspaceExternalProjects.tsx) does NOT use these: it adds or removes
// one pair at a time against the current set, so every other provider's rows — and any row belonging to
// a connection whose list failed to load — are carried through verbatim rather than reconstructed.
// These stay because a bulk provider-scoped replace is still the right shape for a plugin doing one,
// and they are the plugin-facing form exported through @acorn/plugin-api/client.
