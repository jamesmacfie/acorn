...
import { replaceWorkspaceExternalProjectsForProvider, workspaceExternalProjectsForProvider } from './workspaceProjects'
...
const projects: WorkspaceExternalProject[] = [
...
describe('provider-owned workspace project mappings', () => {
  it('selects only mappings belonging to the requested provider', () => {
    expect(workspaceExternalProjectsForProvider(projects, integrations, 'rollbar')).toEqual([
      { integrationId: 'rollbar-a', externalId: 'rollbar-project' },
    ])
  })

  it('replaces one provider without deleting sibling-provider mappings', () => {
    expect(replaceWorkspaceExternalProjectsForProvider(projects, integrations, 'rollbar', [
