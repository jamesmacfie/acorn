...
// One draft panel, held in memory, with every rule the two presentations share. See
// docs/dashboards.md § The generated editor.
//
// It exists because there are two ways to compose a panel, the single-sheet editor and the staged
// wizard, and "two presentations, one truth" is only true if the truth lives somewhere neither of them
// owns. Everything here is form state and the handlers over it. Every rule that can be wrong is a pure
// function in `editor.ts`, `chart.ts` or `mapping.ts` and merely called from here, because vitest in
// this repo runs in node with no Solid plugin and a signal is not testable.
//
...
export function createPanelDraft(props: {
  ...
  // make the cached-schema lookup below depend on where the rail happens to be pointing.
  const nodeId = activeNodeId() ?? ''
  // The panel's identity across every placement that references it, so an edit keeps it and only a new
  // panel mints one (persist.ts, the Perses split). Minted here rather than on commit because the
  // preview renders a definition and a definition has an id; nothing is written until commit.
  ...
  // Set by the first keystroke in the title box. Until then the title follows the collection, which is
  // what makes picking one and pressing Add work without typing anything.
  ...
  // response has nothing here until a panel over it has drawn once, which the cold notice says out loud
  // rather than presenting an empty form.
  //
  // Reactive, through a cache subscription (data.ts, `createCollectionCacheRevision`), because
  // `getQueryData` is only a snapshot. An answer landing while a surface is open, because the panel was
  // just placed or a sibling panel over the same collection fetched, has to fill the gated sections and
  ...
  // select and the definition agree. A control showing a value the panel does not hold is how a person
  // saves something other than what they read.
  if (view().kind === 'board' && !fieldById(shaping().groupBy)) patch({ groupBy: defaultGroupBy(schema()) })

  // ── Sources ───────────────────────────────────────────────────────────────────────────────────

  /** The collections still addable, filtered. The picker's list and the wizard gallery's, one derivation
  ...
  const addSource = (entry: CollectionContribution) => {
    ...
    if (first) {
      // Everything shaped against a collection that is no longer here goes. `normalizePanel` would drop
      // the stale keys on save anyway, but leaving them on screen means showing a person filters over
      // fields their panel does not have.
      setShaping({})
      setLimitText('')
    }
    // The role pre-fill, and the only reason the role vocabulary exists. The host proposes "your title
    // is that source's title, your status is its status" and writes the proposal into the config, where
    // the Fields matrix shows what it decided. It never guesses silently and it never overwrites an
    // answer the person already gave. See docs/dashboards.md § The two vocabularies, and the budget.
    setMapping((current) => {
  ...
  const removeColumn = (index: number) => setMapping((current) => {
    ...
    // The column's own value mappings go with it. An entry keyed by a column that no longer exists is
    // unreachable config, and `pruneMapping` would drop it on save anyway.
  ...
  const addField = () => setMapping((current) => ({
    ...
    extraFields: [
      ...
      // invented fields, a repo name and an issue identifier, are both text.
  ...
  const removeField = (index: number) => setMapping((current) => {
    ...
    // Its per-source answers go with it. `pruneMapping` would drop them on save anyway, and doing it
  ...
  // it up" from needing two pieces of state that can disagree.
   ...
   *  rather than here. The sheet's segmented control and the wizard's cards apply the same rules to the
   *  same pair, and a second copy is how two presentations come to disagree about what choosing a view
   *  means. */
  ...
  /** The enums this chart may be split into series by: every one for a line, every one but the category
   *  axis for a bar (chart.ts, `chartSeriesFields`). Empty means the control is not offered, which is
   *  how the grouped bar stays unrepresentable over a single-enum schema. */
  ...
  const chooseShape = (next: ChartShape) => setView((current) => {
    ...
    // A line split by the enum that just became the bar's category axis is a split `buildBar` drops
    // anyway, so dropping it here too means the select is never showing a value the panel does not hold.
    return { ...current, shape: next, x, ...(current.series && current.series === x ? { series: undefined } : {}) }
  })

  const trends = createMemo(() => trendsFor(schema()))
  /** Turning the trend off takes the comparison with it: a delta with no series behind it is config that
   *  renders nothing, and it would come back the next time somebody flicked the trend on. */
  ...
  /** The draft, as the definition it would be saved as, normalised, so what the preview draws and what a
   *  commit writes are the same object and cannot drift. */
  ...
  /** What a view component needs to draw this draft, over the same compose, mapping and shaping pipeline
   *  a placed panel runs (data.ts, `createPanelData`), from cached rows only. There is exactly one way to
   *  draw a panel and this is it, minus the fetching. */
  ...
  const preview = {
    ...
    /** Whether this device has ever read one of these sources. Answered, not "has rows": a collection
     *  that legitimately returned nothing has been read, and drawing "not read yet" over it would be the
     *  cold notice lying. `pages` subscribes to the cache revision, so this ticks when an answer lands
     *  with the surface open. */
  ...
  return {
    existing,
    nodeId,
    /** Ticks when a collection page lands in this node's cache. Exposed for the one reader that looks at
     *  collections the draft does not hold yet, the wizard's gallery cards, whose row counts and read-at
     *  times come out of the same cache (data.ts, `createCollectionCacheRevision`). */
    cacheRevision,
