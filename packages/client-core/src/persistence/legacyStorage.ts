...
// credentials in plaintext. Dropping the writer stops new ones appearing and does nothing about
// the bytes already on the device, and the device that matters is the one upgrading straight from
// the release that wrote them.
//
// This used to be a plugin's own `activate` hook: http purged `http-draft:*` before its panel
// could read one back. It is here now because http ships as a loaded plugin: a frame's
// `localStorage` is its own, keyed by bundle hash at an `app-plugin://` origin, so a purge running
// inside the frame would sweep a storage area that never held these keys and report success. The
// bytes are the shell's, and so is the cleanup, which is the better home for it anyway, since what
// is being cleaned up is a decision an old version of the app made, not a decision a plugin made.
