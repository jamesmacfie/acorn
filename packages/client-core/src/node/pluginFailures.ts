...
//
// It is an attention item, not a notice (registries/attention.ts): this is a state, not an event. The
// plugin stays broken across refetches until the owner disables it or fixes it, so dismissing it and
// having it come back is correct.
//
// 'load' means the package never ran at all: its manifest did not parse, its bundle did not import, or
// its apiVersion is not this node's. Saying "failed to start" for that would be the small lie the old
// wording told.
//
// A row that carries a reason while its state is not 'failed' is the third case: a package on disk
// claimed a name something else already answers to, a dogfooded copy of a built-in, or a second
// directory claiming a won id. What is running is fine, so this is not a failure of the plugin; it is
// a failure of the copy the owner installed, and saying "failed to load" would send them looking at
// the wrong thing.
...
// The generic sentence, used only when the node is too old to send a reason. Everything a newer node
// sends is more specific than this by construction.
...
export const pluginFailureAttention: AttentionSourceContribution = {
  ...
  // Above the plugins' own sources: a plugin that did not start is more urgent than anything a plugin
  // that did start has to say.
  order: 5,
  fetch: async (nodeId, signal) => {
    ...
    const items: AttentionItem[] = state.plugins
      ...
      // a row whose own state is honestly 'active', so filtering on state alone dropped it.
      .filter((row) => row.state === 'failed' || row.reason !== undefined)
      .map((row) => ({
        id: `core.pluginFailures:${row.name}`,
        title: titleFor(row),
        // What actually broke, in the words of whatever broke it. Interpolated as text, since this is
        // a loaded plugin's own thrown message crossing into the owner's UI. It is never markup, and
        // the node has already capped its length (node-core/server/plugin/pluginState.ts).
        //
        // No restart advice either way: a plugin that threw during init runs the same code again, and
        // a bundle that will not import will not import next boot either.
        ...
        //
        // `Date.now()`, not 0, when it is genuinely absent, which now only happens against a node too
        // old to send it. `at` is required and feeds the relative time and the newest-first sort, so 0
        // rendered every load failure as a 56-year-old event that also sorted last in its band. The
        // cost of this fallback is that such a row does reset to "just now" each poll; that is the
        // lesser lie, and it disappears once the node is updated.
