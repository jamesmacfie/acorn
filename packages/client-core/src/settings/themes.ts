 ...
 * disabled, uninstalled, its client bundle stops being trusted, its node is unreachable, or the author
 * renames the theme. Applying an unknown id would set `data-theme` to a value no block matches, which
 * renders the `:root` default palette while `--is-dark` says whatever the OS media block last said —
 * a half-applied theme rather than a missing one.
 *
 * So the read falls back and the WRITE is left alone. Nothing here clears the pref, deliberately:
 * "the owning plugin is gone" and "the node is having a bad minute" arrive at this function as exactly
 * the same absence, and a pref erased on the second one cannot be recovered when the node comes back.
 * A user who sees Light for a minute and their own theme afterwards has lost nothing; a user whose
 * choice was deleted has to go and find it again.
 *
