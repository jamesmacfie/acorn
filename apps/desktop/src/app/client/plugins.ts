...
// `sourceRegistry.entries()` in registration order — client-core/tabs/sources.ts puts GitHub first and
// appends the rest unsorted — so the rail today reads GitHub, Linear, Rollbar, Docker, API, Agents, and
// the first five entries below are in that order to keep it. Everything else sorts on its own field:
...
// Fourteen of the twenty plugins. The other six have no client contributions to make, which is the
...
//     registries.
//   memory — has client code (MemorySection, memoryClient) but registers nothing: it renders INSIDE
//     context's pane, which imports it directly. That import is one of the six edges on the boundary
//     ledger and Phase 3 turns it into a context-section contribution; a ClientPlugin shell today would
//     hold an empty init.
//   onboarding — same shape. App.tsx renders OnboardingModal against its own first-run signal, so the
//     modal has no slot to register into until the shell stops rendering feature UI (Phase 3, row 1).
export const clientPlugins: readonly ClientPlugin[] = [
  // Order-sensitive block: these five own the rail's five non-GitHub sources, in rail order.
  linearClientPlugin,
  rollbarClientPlugin,
  dockerClientPlugin,
  httpClientPlugin,
