...
// Device-token custody, in Electron main (docs/architecture-overview.md § How the client talks to nodes):
// the renderer never holds a token, so the credential lives here and only here.
//
...
//
// One file per scope, where a scope is a nodeId, except the bundled local node, which uses the constant
// below. The local node can't be keyed by nodeId, because its token has to be read before the service
// starts and starting it is the only thing that can tell us the nodeId. There's exactly one bundled node
// per userData dir, so the dir is its scope. fleetStore.ts owns that mapping.
//
// A stale token, where the data root was wiped so the node no longer knows it, needs no detection here:
...
// unavailable, ciphertext from a different keychain entry) returns undefined, because the recovery is
