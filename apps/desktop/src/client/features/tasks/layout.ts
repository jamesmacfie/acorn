...
export type PaneId = 'pr' | 'linear' | 'rollbar' | 'preview' | 'editor' | 'changes' | 'notes' | 'context' | 'database' | 'search'
...
export const PANE_ORDER: readonly PaneId[] = ['pr', 'changes', 'notes', 'context', 'editor', 'search', 'database', 'preview', 'linear', 'rollbar']

export const DEFAULT_PANE: PaneId = 'pr'
export const defaultLayout = (pane: PaneId = DEFAULT_PANE): TaskLayout => ({ panes: [pane] })

const PANE_IDS: readonly PaneId[] = ['pr', 'linear', 'rollbar', 'preview', 'editor', 'changes', 'notes', 'context', 'database', 'search']
