...
async function pairTwoNodes(options: { seedNode?: (dataDir: string) => void } = {}): Promise<TwoNodes> {
  ...
  // Before the second node exists, so before anything it serves can raise a trust prompt over the
  // first-run wizard. From here the preference is on disk and the reload below starts without it.
