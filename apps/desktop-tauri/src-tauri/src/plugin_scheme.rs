...
// `app-plugin://<sha256>/...` is the origin a plugin's UI runs on, and this is the Tauri half of
// `apps/desktop/src/app/main/pluginScheme.ts`. Why the hash is the host, what it buys (isolation,
// immutability, custody), and why index.html is generated here rather than shipped by the plugin:
// docs/electron.md § The plugin frame origin, docs/plugins.md.
//
...
//
// Two things differ from the Electron file, both because Rust serves this rather than main:
//
...
//
// The frame stylesheet is a staged file rather than a `?raw` import, read once at boot. Same bytes,
// same order — `scripts/stage.mjs` builds it from the list in `pluginFrameStyles.ts`, so the two
// shells cannot disagree about what a frame is dressed in.
...
/// docs/electron.md § The plugin frame origin. `connect-src 'none'` is the load-bearing one, and
...
/// The generated document, byte-identical in meaning to `pluginScheme.ts`'s. Deliberately tiny: the
...
///
/// The inline block sits before the stylesheet link, and that order matters — the Electron file
/// carries the long version of why, and the short one is that a later `html, body` rule beat
/// `base.css` on source order alone and rendered every frame in the browser's default serif.
