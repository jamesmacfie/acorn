...
import { decodeBytes, encodeBytes, type HelperMessage, type HelperMethod, type HelperPush, type HelperRequest, type WireFetchRequest } from '../shared/wire'
import {
  decisionSchema,
  devGrantSchema,
  disclosureSchema,
...
// of what `apps/desktop/src/app/main/nodeBrokerIpc.ts` and `pluginIpc.ts` are under Electron: same
  ...
  const handlers: Record<HelperMethod, (params: unknown) => unknown | Promise<unknown>> = {
    'node-fetch': async (raw) => {
      ...
      try {
        ...
        // already stopped caring (apps/desktop/src/app/main/nodeBrokerIpc.ts has the long version).
    ...
    'node-send': (raw) => {
      ...
      // Structural check only, for the reason nodeBrokerIpc.ts gives: the frame vocabulary is a
