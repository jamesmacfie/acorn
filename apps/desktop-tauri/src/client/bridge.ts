...
// The Tauri shell's answer to `apps/desktop/src/app/main/preload.ts`: it assembles the same
// preload-shaped object the platform seam reads and installs it before the shell mounts. The seam
// (`packages/client-core/src/platform/index.ts`) and the arch rule that guards it need zero changes;
// this is a second host for a contract that already exists.
//
