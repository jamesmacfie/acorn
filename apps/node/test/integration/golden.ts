...
// inside it. Mounting a route or contributing a tool, a provider or a database used to fail a test whose fix
// was "find the table, hand-add a row"; now it fails a test whose fix is one command:
//
...
// order-sensitive. `JSON.parse` round-trips that with no parser of ours to get wrong, and pretty-printed it
// still diffs one line per entry.
//
// Ceiling: these fifteen lines exist twice, once per app (see apps/desktop/test/client/golden.ts). Two apps
// must not import each other's test helpers, and a shared package for fifteen lines would cost more than the
// duplication does.
