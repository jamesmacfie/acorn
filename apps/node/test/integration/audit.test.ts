...
beforeEach(async () => {
  ...
  disabled = []
  // The plugin toggle goes through a bridge, because the roster only exists once the composition
  // root has run the plugin host. Two plugins is enough to show the recorded list is the state, not
  // a diff.
...
// Functions, not consts: `deviceToken` is minted per case in beforeEach, and a module-level const
// would capture the value from before the first one ran (i.e. undefined, i.e. a 401 on every
// request).
...
describe('the audit trail records what security.md says it should', () => {
  it('already holds the pairing that created this device', async () => {
    // Written by deviceService.issue, not by a route. That's why it is here at all: the bundled
    // local node pairs with no code, so a route-only producer would miss the most common pairing on
    // any machine.
