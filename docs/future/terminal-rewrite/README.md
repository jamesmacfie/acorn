...
| [3](./phase-3-widgets-and-the-pty.md) | The scroll viewport, `Input`, `Textarea`, mouse hit testing, and the `pty` rectangle over `@xterm/headless`. | The scroll viewport is built, 2026-09-04: the offset, the bar, the wheel, one reveal instead of two, and Yoga's `flexShrink` default settled. Fourteen findings are at the bottom of the phase file. The other three widgets and the parser's wiring are next. |
