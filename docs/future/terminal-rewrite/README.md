...
| [2](./phase-2-the-painter.md) | The node tree, the Yoga pass, the cell buffer, the diff, and the flush, behind a build switch so both painters run against the golden frames. | Built 2026-09-04. Under `ACORN_TUI_PAINTER=own` the new painter draws the whole app on Node 24.11.0 with no FFI, and 16 of the 28 goldens match cell for cell; the other 12 wait on a widget. Fifty-three false premises are recorded at the bottom of the phase file. |
| [3](./phase-3-widgets-and-the-pty.md) | The scroll viewport, `Input`, `Textarea`, mouse hit testing, and the `pty` rectangle over `@xterm/headless`. | Two slices built, 2026-09-04. The scroll viewport: the offset, the bar, the wheel, one reveal instead of two, and Yoga's `flexShrink` default settled. Then `Input` and `Textarea` over a model of ours: typing reaches a field under our painter, and 23 of the 28 goldens now match cell for cell. Twenty-eight findings are at the bottom of the phase file, in two sections. The `pty` rectangle, the parser's wiring and the click hit test are next. |
| [4](./phase-4-cut-over.md) | Remove OpenTUI, the FFI gate, the Node 26 floor, and the seven workarounds; rewrite the test harness; move the behaviour into `docs/tui.md`. | Two slices built 2026-09-04. The first: the switch, the FFI gates and the harness gone, the whole suite running and drawing on Node 24.11.0 with no flag, and the goldens settled. The second: the workaround files, both painter packages, twenty dead dependencies behind three throwing stubs, the goldens themselves, an honest startup ceiling and the repo's own `engines`. 557 passing, 2 failing, 2 skipped, and both failures are somebody else's. Only the docs are left. |
| [5](./phase-5-what-the-reference-apps-do.md) | The few conventions the reference apps have that we lack, taken up against the keyboard rules rather than copied. | Built 2026-09-04. The cheat sheet and `G` were already there, so those two items were tests rather than changes; digits, `g g` and a region label in the footer are refused with their reasons. The one real fault was the footer's own: `esc back` was in the list at every depth and cut off the line at all of them. Findings at the bottom of the phase file. |
