...
| [4](./phase-4-cut-over.md) | Remove OpenTUI, the FFI gate, the Node 26 floor, and the seven workarounds; rewrite the test harness; move the behaviour into `docs/tui.md`. | First slice built 2026-09-04. The switch, the FFI gates and the harness are gone, and the whole suite runs and draws on Node 24.11.0 with no flag: 584 passing, 2 failing, 5 skipped. Twenty-five of the 28 goldens match, two were re-captured and three are accepted. The workaround files, the packages, the dead dependencies, the goldens and the docs are the slices left. |
