...
| [2](./phase-2-the-painter.md) | The node tree, the Yoga pass, the cell buffer, the diff, and the flush, behind a build switch so both painters run against the golden frames. | Part built 2026-09-03: the tree, the width measure, the layout pass, the colour type, the paint pass and the input parser. The switch and the golden comparison remain. Thirty-six false premises are recorded at the bottom of the phase file. |
