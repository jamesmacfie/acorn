...
A programme, 2026-09-02. Phase 1 has shipped and nothing else has started. It starts from a
...
| [phase-2-settings.md](./phase-2-settings.md) | Settings → Notifications: one device preference, six checkboxes, a test button. |
| [phase-3-sound.md](./phase-3-sound.md) | Two synthesised chimes on the desktop, BEL on the terminal client. |
...
| 2 | A Notifications settings page over one JSON device preference, with a test button. | 1 | Not started. |
| 3 | Two WebAudio chimes; BEL on the terminal client. | 2 | Not started. |
...
Phase 2 is next. Phases 3, 4, and 5 are independent of each other once 2 has landed, and 5 needs
only 1 if BEL is deferred. Phase 1's gate reads `DEFAULT_NOTIFICATION_SETTINGS` in `deliver.ts`,
which is every switch on; phase 2 replaces that reader with the device preference.
...
- `docs/features.md` § Settings and fleet lists the settings sections; phase 2 adds one.
- `docs/state-ownership.md` § Scope rules puts notices and settings on the device; phase 2 leans on
  it and phase 6 adds a line.
