...
Build in this order. Phases 1 and 2 may run in parallel; nothing else may be reordered.

| Phase | What | File | Why it is first |
| --- | --- | --- | --- |
| 0 | Model keys + codec + pure derivations; the measure-history store; the series-colour decision; the `source` panel-local field | `measure-history.md`, `wizard.md § Foundation`, `charts.md` | Everything later renders from these. Building UI first means rebuilding it when the shapes land. |
| 1 | Grid gesture + panel chrome restyle | `ux-refresh.md` | Pure presentation; touches no data. Parallel-safe with phase 0. |
| 2 | The panel wizard | `wizard.md` | Needs phase 0's derivations (`viewAvailability`, picker metadata, size presets). |
| 3 | Stat trend + delta rendering; chart growth (legend, grouped bar, source split, sparkline mark) | `measure-history.md § Display`, `charts.md` | Needs the history store populated and the series-colour decision made. |
...
| [`measure-history.md`](./measure-history.md) | Host-side measure sampling: the store, the routes, and the stat delta/sparkline it feeds. | Foundation is ungated; the *display* half waits on the store. |
