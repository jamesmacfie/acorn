...
| `docs/first-party-plugins.md` | 6 partly 2026-08-30, 7 partly 2026-08-30, 8 partly 2026-08-30, 9 | Reason B ("in-realm components inside another surface's tree") dissolves for everything a remote tree can carry; changes, docker's slots, onboarding's overlay, and memory move off the "must be" and "one specific reason" tables. § "First-party only by history" loses editor's blocker. Phase 6 rewrote the argument: the `remote` kind carries a real interface, and memory's section, which the section named as the proof that it could not, is a `context:section` contribution. No table row moved, and the phase file records why. Phase 7 rewrote github's row: nothing about how it draws itself keeps it first-party, only `GITHUB_MIRROR` having a consumer. Phase 8 took `agentToolRenderers` off reason E's list; agents keeps D, E and F, because it still registers a `persistedStateSlices` slice. |
...
| `docs/agent-tools.md` | 6 done 2026-08-30 | § "Context sections" gained § "Drawing inside a section": the slot is `context:section` and memory is its one contributor. Phase 8 found no tool-renderer paragraph left in this file to repoint: `docs/contribution-kinds.md` and `docs/plugins.md` already name `agents:tool-card`, and phase 8 updated both. |
...
| `docs/diff-rendering.md` | 4, 6 partly 2026-08-30, 7 done 2026-08-30 | § "Row geometry" and § "Review threads and state" gain the annotation draw site. Phase 6 opened `changes:diff-line` and gave `lineExtra` a host-owned wrapper (`.diff-line-extra`), so an owner's own annotation and a contributor's marks share one full-width line under the code. Phase 7 opened `github:diff-line` beside it and deleted the pull-request navigator's own scroll store. |
| `docs/github-integration.md` | 7 done 2026-08-30 | New § "Surfaces": what each of the five is made of, the two points github opens, and where its keyboard navigation comes from now. § "Tasks and references" says strip rather than tabs; § "Actions and logs" gains the run modal. |
| `docs/security.md` (again) | 7 done 2026-08-30 | The `bodyHTML` binding list shrank from four to two: three of them were github's, and they are one host component now. |
| `docs/features.md` | 9 | The pane list reflects layouts. |
| `docs/managed-agents.md` | 3 (the tool card, 2026-08-29), 8 done 2026-08-30 | § "Client surfaces": the tool card is a slot; the pane is a `list-detail` layout, the transcript is a `Timeline follow`, and the composer has the `attachment` and `composer-actions` slots. |
