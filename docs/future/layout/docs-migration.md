...
| `docs/panes.md` | 1, 5 done 2026-08-30, 9 | § "Layout model" states the two layers: the task layout row (unchanged) and the pane's declared layout (new). § "Contributions" replaces the component list with `layout` and `regions`, and drops agent-tool renderers and task slots in favour of slots. § "Not a pane: the reference panel" notes the panel body is a tree. Phase 5 rewrote the region table (`remote`, `frame`, `document`), marked the four shipped panes as trees, and added the `single`-only rule for reference panels and settings pages. |
