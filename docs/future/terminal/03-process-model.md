...
The supervise-a-child code is the desktop helper's (`packages/custody/src/supervision/`), and the
helper has no shell binding by design (`packages/custody/src/index.ts`). The TUI imports
...
(`packages/custody/src/broker/nodeBroker.ts`). That split is why the helper is a separate
process.
...
(`packages/custody/src/broker/nodePairing.ts`): an unverified probe of `GET /v2/node` that
