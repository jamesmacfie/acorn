...
| context | Hosts the tray that memory renders into (receiving half of the same coupling). | — | Movable only together with the memory seam below. |
...
| memory | Renders inside context's tray (9 of the 12 CSS cross-package baseline entries); `MEMORY_KNOWLEDGE` capability named by the composition root. | client `contextSections` | Movable once the tray becomes a cooperative extension point (below). |
...
| terminal | Owns PTY streams ("exactly one plugin may own these"); the drawer is a core surface; folder-picker main code. | `drawer`, `topbar.right` slots | **Stays first-party** (stream owner). |
...
1. **memory → context's tray.** The heaviest in-realm coupling in the tree (nine CSS baseline
   entries). This is *exactly* the "cooperative cross-plugin extension point" designed in
   `docs/future/user-extensions/extension-points.md § 3` — plugin A declares a point, plugin B
   contributes descriptors. Build that seam with this pair as its first consumer: a real
   first-party consumer pair, per the build-the-seam-anyway rule, and it un-blocks memory's move
   as a side effect.
2. **workflows → agents (`WORKFLOW_CONTROL`).** Three function references passed in-realm. The
   loaded-tier shape already exists: node-side capabilities plus plugin routes. Decide whether the
   sidebar's controls become descriptors (verbs from the closed set: `runNodeAction`) or a small
   frame. Either removes the last in-realm function passing between plugins.
...
- **Dead slot ids** `topbar.left` and `task.switcher.extra` — zero consumers anywhere.
- The `themes`/`styles` registries are *plugin*-dead (unreachable from any plugin context) but
  core seeds its own twelve themes through them — keep, and note they are the landing point for
  `user-extensions/extension-points.md § 1` (manifest-declared token themes), not dead code.
