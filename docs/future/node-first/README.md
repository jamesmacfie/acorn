...
|---|---|---|---|
| 1 | **There is no client↔node compatibility contract.** One exact-equality version check at pairing time, never re-checked; the `incompatible` state is dead code; the handshake schema rejects unknown fields, so it breaks on *additions*. | Standalone nodes mean permanent version skew. The no-compat window is open now and closes at the first standalone release. | [version-skew.md](./version-skew.md) |
