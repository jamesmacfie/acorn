...
| Extension points | Both | `ctx.contribute(extensionPointRegistry)` / `contributions.extensionPoints` | A surface a plugin opens to others, in one of five kinds: rows, annotations, remote trees, rectangles, hooks (docs/plugins.md § Cooperative extension points) |
| Extensions | Both | `ctx.contribute(extensionRegistry)` / `contributions.extensions` | A contribution into someone else's point, carried as `items`, `remote`, `frame` or `route` |
