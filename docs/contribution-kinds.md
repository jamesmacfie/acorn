...
| Extension points | Both | `ctx.contribute(extensionPointRegistry)` / `contributions.extensionPoints` | A surface a plugin opens to others |
| Extensions | Both | `ctx.contribute(extensionRegistry)` / `contributions.extensions` | A contribution into someone else's point |
...
| Extension points | Both | `ctx.extensionPoints` | The node's many-to-many seam: one plugin opens a point, any number fill it |
