...
The right ("Diff") pane renders **every** changed file's diff stacked in one scroller. It is the most involved part of the [frontend](./frontend.md): patch text from the [GitHub integration](./github-integration.md) is parsed, syntax-highlighted, virtualized, hydrated in priority order, optionally split into two columns, given word-level intra-line diffs, expandable hidden context, and interleaved inline review threads. `DiffView.tsx` owns route/query/scroll orchestration (it renders one keyed `DiffForPull` per routed PR; session-only scroll state is restored per task/PR or classic-browse PR); pure model work lives in `client-core/ui/diff/model.ts`, the priority hydrator in `client-core/ui/diff/hydration.ts`, virtualizer construction + batched measure scheduling in `client-core/ui/diff/virtualization.ts`, row rendering in `client-core/ui/diff/DiffRows.tsx`, and shared helpers beside them. `ComparePreview` (create mode's read-only base..head preview) reuses the same hydrator + row model, minus threads/composers/virtualization.

The viewer itself is **shared UI**, not the github plugin's: it lives in `@acorn/client-core` (`ui/diff/*`) because the [changes pane](./panes.md) renders a worktree's uncommitted diff through the same rows. Only route/query orchestration (`DiffView.tsx`, `ComparePreview.tsx`) stays in the github plugin.
...
GitHub's per-file `patch` is **hunks-only** — it has no `diff --git` / `---` / `+++` file header, so a parser keyed on that header sees nothing. `client-core/ui/diff/synth.ts` synthesizes one:
...
`client-core/ui/diff/model.ts` owns the pure diff model. `buildDiffRows(file, tokenize)` feeds `synth(...)` to `gitdiff-parser`, then flattens the parsed hunks into a flat `DiffRow[]`:
...
Parsing + highlighting every file up front would block the main thread on a large PR. Instead `createDiffHydrator` (`client-core/ui/diff/hydration.ts`) parses/tokenizes files **in priority order, off the render path**:
...
Threads render as full-width `ThreadRow`s from `client-core/ui/diff/DiffRows.tsx` (shared by both view modes via `NonCodeRow`): each comment, a Resolve/Unresolve toggle, a Show/Hide collapse, and a reply box (replies target the thread's first comment `databaseId`). A hover **`+`** on any code line opens a `LineComposer` to start a new line comment — enabled only when `headSha` is known and the line has a number. Composers get an @mention autocomplete fed by `mentionsOptions`. Both `addReviewComment` and `replyReview`/`resolveThread` invalidate `['pull', …]` on success (see [frontend](./frontend.md) for the mutation pattern).
