...
const mutationResult = async (operation: Promise<void>): Promise<{ ok: true }> => {
  await operation
  return { ok: true }
}
...
export const notes = new Hono<AppEnv>()
  ...
  .put('/workspaces/:wsId/notes/:slug', async (c) => {
    ...
    if (!parsed.success) return respondError(c, 400, 'bad_request')
    return viaBridge(c, NOTES_STORE, (store) => mutationResult(store.write(workspaceLocation(c.req.param('wsId')), c.req.param('slug'), parsed.data.body)))
  })
  .post('/workspaces/:wsId/notes/:slug/included', async (c) => {
    const parsed = includedBody.safeParse(await c.req.json().catch(() => null))
    if (!parsed.success) return respondError(c, 400, 'bad_request')
    return viaBridge(c, NOTES_STORE, (store) => mutationResult(store.setIncluded(workspaceLocation(c.req.param('wsId')), c.req.param('slug'), parsed.data.included)))
  })
  .post('/workspaces/:wsId/notes/:slug/title', async (c) => {
    const parsed = titleBody.safeParse(await c.req.json().catch(() => null))
    if (!parsed.success) return respondError(c, 400, 'bad_request')
    return viaBridge(c, NOTES_STORE, (store) => mutationResult(store.setTitle(workspaceLocation(c.req.param('wsId')), c.req.param('slug'), parsed.data.title)))
  })
  .delete('/workspaces/:wsId/notes/:slug', (c) => viaBridge(c, NOTES_STORE, (store) => mutationResult(store.remove(workspaceLocation(c.req.param('wsId')), c.req.param('slug')))))
  ...
  .put('/tasks/:id/notes/:slug', async (c) => {
    const parsed = writeBody.safeParse(await c.req.json().catch(() => null))
    if (!parsed.success) return respondError(c, 400, 'bad_request')
    return viaBridge(c, NOTES_STORE, (store) => mutationResult(store.write(taskLocation(c.req.param('id')), c.req.param('slug'), parsed.data.body)))
  })
  .post('/tasks/:id/notes/:slug/included', async (c) => {
    const parsed = includedBody.safeParse(await c.req.json().catch(() => null))
    if (!parsed.success) return respondError(c, 400, 'bad_request')
    return viaBridge(c, NOTES_STORE, (store) => mutationResult(store.setIncluded(taskLocation(c.req.param('id')), c.req.param('slug'), parsed.data.included)))
  })
  .post('/tasks/:id/notes/:slug/title', async (c) => {
    const parsed = titleBody.safeParse(await c.req.json().catch(() => null))
    if (!parsed.success) return respondError(c, 400, 'bad_request')
    return viaBridge(c, NOTES_STORE, (store) => mutationResult(store.setTitle(taskLocation(c.req.param('id')), c.req.param('slug'), parsed.data.title)))
  })
  .delete('/tasks/:id/notes/:slug', (c) => viaBridge(c, NOTES_STORE, (store) => mutationResult(store.remove(taskLocation(c.req.param('id')), c.req.param('slug')))))
