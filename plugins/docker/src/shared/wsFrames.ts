...
// One sample per `docker stats` tick (~1s).
...
// Renderer → node. Refs are shape-validated at the handler (main/wsChannel.ts) before they reach argv.
...
// Node → renderer. `docker:changed` is the cache-dirty ping; its scopes are
