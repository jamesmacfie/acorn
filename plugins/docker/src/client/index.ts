...
export const dockerClientPlugin: ClientPlugin = {
  name: 'docker',
  init: (ctx) => {
    // The other feeder. A loaded plugin declares its mark in the manifest and the host registers it;
    // compiled in, we contribute it directly — and every `brand:docker` glyph string below reads the
    // same either way, which is the point of the mark being data rather than a component.
    //
    // Through `ctx.contribute` and not `brandMarkRegistry.register`, so the host records the
    // disposable exactly as it does for the panes and slots below. A bare register would survive a
    // disable/enable cycle and make the next activation throw on the duplicate id.
    // From simple-icons (CC0 artwork; the trademark remains Docker's).
