...
// open state, toggle, and whether a task is active at all. Moved out of
// apps/desktop/src/app/client/slotContributions.tsx, where the shell contributed the toggle for the
// drawer this plugin owns.
...
export const terminalToggleSlotContribution: UiSlotContribution = {
  ...
  slot: 'topbar.right',
  order: 20,
  // The drawer needs the main-process PTY engine, and there's nothing to toggle without a task.
  requires: 'desktop',
  when: (context) => context.taskActive,
  component: (props) => (
    // Was borrowing `.theme-toggle`, a class belonging to a control this plugin doesn't own.
    <ToggleButton
      variant="bare"
