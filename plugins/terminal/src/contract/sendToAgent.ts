...
// agent session, which means writing into a PTY the terminal plugin owns. Importing the engine would be
// a memory-to-terminal coupling edge, so the composition root used to supply `sendToAgent` as an
// app-level dep until terminal became a NodePlugin. It is one now, so this is that capability.
//
...
// Fire and forget by design: 'after-ready' doesn't resolve until the session next goes idle, which may
// be minutes away or never. Delivery failures (no such session, session exited) are swallowed for the
// same reason, since the only caller is best-effort launch injection, which must never block or fail a
// session launch.
