...
describe('memories_fts schema drift guard', () => {
  ...
  it('FTS5 columns match the indexed memories columns', async () => {
    ...
    // id is UNINDEXED, being the join key back to memories; name, description and body are the search
    // surface.
    expect(ftsColumns).toEqual(['id', 'name', 'description', 'body'])

    // Every FTS column must still exist on memories, which schema.ts is what the migrations mirror.
