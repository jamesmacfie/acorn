...
//
// This swaps the markdown source, not the rendered DOM. `renderMarkdown` is a pure host function the
// frame drops through `innerHTML`, so rewriting afterwards means walking a container that gets
// rewritten whenever anything upstream ticks. Substituting the URL first keeps the whole thing a
// value, and re-renders fall out of the signal.
//
...
/** The same text with every resolved upload replaced by its data URL. An unresolved one is left
 *  alone, which renders as a broken image. */
