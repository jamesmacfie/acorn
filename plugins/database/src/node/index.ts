...
import { databaseBridge, type DatabaseBridge } from '../server/database'
...
export const databasePlugin = (): NodePlugin => {
  let bridge: DatabaseBridge | null = null
  return {
    name: 'database',
    init: (ctx) => {
      ...
      const data = databaseBridge(ctx.core, ctx.events.send)
      bridge = data
      ...
      const services = { db, core: ctx.core, bridge: data, query }
    ...
    // Release the task connections this plugin opened. The pools themselves belong to core; this
    // adapter only remembers its leases and never receives the underlying handles.
