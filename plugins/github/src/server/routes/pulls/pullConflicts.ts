...
import type { PullConflicts } from '../../../shared/api'
...
// this to PRs it has flagged as conflicting, so the fetch and network hop only happen when there is
// actually a conflict.
...
export const pullConflicts = (core: Pick<CoreServices, 'projects' | 'git'>) => new Hono<AppEnv>().get('/:owner/:repo/pulls/:number/conflicts', async (c) => {
  ...
  // checkout's own branches. `pull/<n>/head` resolves fork heads too. Best-effort: a network or fetch
  // failure degrades to "unavailable" rather than erroring the PR screen.
  ...
  try {
    ...
    // Exit 1: conflicts, and the conflicting paths are on stdout. Anything else means it couldn't
    // compute.
