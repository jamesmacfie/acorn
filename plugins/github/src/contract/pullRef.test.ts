...
// the collection row id, the URL recogniser's `item`, the reference panel parsing it back, and the
// "is there already a task for this?" check.
...
describe('the pull-request reference', () => {
  ...
  it('refuses anything that is not one', () => {
    // A panel handed a stranger's displayId has to be able to say so rather than render an empty shell.
...
describe('matching a reference to a task', () => {
  ...
  it('matches the task that already tracks this pull request', () => {
    ...
    // Same number, different repository — which is the whole reason the reference carries the repo.
    expect(pullRefMatchesTask('runn-fast/other#8811', github, 8811)).toBe(false)
  })

  // THE CASING TRAP, for the third time in this area and pinned here so it is the last. Core's `projects`
  // stores the owner folded (`runn-fast`), github's own mirror and every URL keep GitHub's canonical
  // spelling (`Runn-Fast`), and a task row carries whichever core gave it. An exact match fails silently
  // and looks exactly like "no task for this PR" — so the panel would offer to create a duplicate.
